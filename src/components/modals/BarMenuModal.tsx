...
import { BarMenuItem, Category } from '../../store/slices/barMenuSlice';

interface BarMenuModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: any) => Promise<void>;
  item?: BarMenuItem | null;
  categories: Category[];
}

export default function BarMenuModal({ isOpen, onClose, onSave, item, categories }: BarMenuModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFormError(err || 'Failed to save bar menu item');
    } finally {
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold">{item ? 'Edit Bar Menu Item' : 'Add New Bar Item'}</h2>
